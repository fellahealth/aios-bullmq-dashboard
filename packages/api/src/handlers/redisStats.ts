...
import { DashboardRequest, ControllerHandlerReturnType, RedisStats } from '../../typings/app';
...
export async function redisStatsHandler({
  ...
  uiConfig,
}: DashboardRequest): Promise<ControllerHandlerReturnType> {
  ...
  const pairs = [...queueRegistry.values()];
